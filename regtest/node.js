...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.berycoind.height === 150) {
            node.services.berycoind.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.berycoind.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('berycoind/hashblock', function(data) {
        bus.unsubscribe('berycoind/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.berycoind.height;
      node.services.berycoind.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.berycoind.sendTransaction(tx.serialize(), function(err, hash) {
