...
var BerycoinService = require('../../lib/services/berycoind');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each berycoincore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'berycoind',
          module: BerycoinService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each berycoincore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'berycoind',
