...
var bitcore = require('berycoincore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var BerycoinRPC = require('berycoind-rpc');
...
/**
 * Provides a friendly event driven API to berycoind in Node.js. Manages starting and
 * stopping berycoind as a child process for application support, as well as connecting
 * to multiple berycoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Berycoin(options) {
  if (!(this instanceof Berycoin)) {
    return new Berycoin(options);
  }
...
util.inherits(Berycoin, Service);

Berycoin.dependencies = [];

Berycoin.DEFAULT_MAX_TXIDS = 1000;
Berycoin.DEFAULT_MAX_HISTORY = 50;
Berycoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Berycoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Berycoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Berycoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Berycoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Berycoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Berycoin.DEFAULT_REINDEX_INTERVAL = 10000;
Berycoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Berycoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Berycoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Berycoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Berycoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Berycoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Berycoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Berycoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Berycoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Berycoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Berycoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Berycoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Berycoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Berycoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Berycoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Berycoin.prototype._initCaches = function() {
...
Berycoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Berycoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Berycoin.prototype.getPublishEvents = function() {
  return [
    {
...
Berycoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'berycoind/' + name, 'total:', this.subscriptions[name].length);
};

Berycoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'berycoind/' + name, 'total:', this.subscriptions[name].length);
};


Berycoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'berycoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Berycoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'berycoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Berycoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Berycoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'berycoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Berycoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'berycoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Berycoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Berycoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Berycoin.DEFAULT_CONFIG_SETTINGS;
...
Berycoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Berycoin.prototype._expandRelativeDatadir = function() {
...
Berycoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in berycoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in berycoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in berycoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './berycoin.conf');

  log.info('Using berycoin config file:', configPath);
...
Berycoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of berycoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of berycoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of berycoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to berycoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from berycoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from berycoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that berycoind is undergoing a reindex. ' +
             ...
             'of berycoincore-node services will start.');
    node._reindex = true;
  }
};

Berycoin.prototype._resetCaches = function() {
...
Berycoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Berycoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Berycoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Berycoin Daemon Ready');
          callback();
        });
      });
...
Berycoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9947
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 19947;
  }
  return networkOptions;
};

Berycoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/berycoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/berycoin.conf';
    }
  }
  return networkPath;
};

Berycoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Berycoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('berycoind/hashblock', message.toString('hex'));
    }
  }

};

Berycoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Berycoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Berycoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Berycoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Berycoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Berycoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('berycoind/addressbalance', {
                    address: address,
                    txid: txid,
...
Berycoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('berycoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Berycoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('berycoind/rawtransaction', message.toString('hex'));
    }
...
Berycoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Berycoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Berycoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Berycoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Berycoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Berycoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Berycoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Berycoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Berycoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/berycoind.pid';
...
Berycoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting berycoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Berycoin process unexpectedly exited with code:', code);
        log.warn('Restarting berycoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Berycoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn berycoind.'));
      }
...
Berycoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to berycoind.'));
    }
 ...
 */
Berycoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Berycoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Berycoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Berycoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, berycoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Berycoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Berycoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Berycoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Berycoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in berycoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Berycoin.prototype._getConfirmationsDetail = function(transaction) {
...
Berycoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Berycoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Berycoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A berycoincoin transaction id
 * @param {Function} callback
 */
Berycoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Berycoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Berycoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Berycoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Berycoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Berycoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Berycoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getInfo = function(callback) {
...
Berycoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('berycoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('berycoind process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of berycoin addresses to filter
 * @param {Function} callback
 */
Berycoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Berycoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Berycoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Berycoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Berycoin.prototype.getStakingInfo = function(callback) {
    var self = this,
