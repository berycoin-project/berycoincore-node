...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for berycoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and berycoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed berycoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './berycoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('Berycoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in berycoincore-node services directory
    ...
    if (servicePackage.berycoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.berycoincoreNode;
    }
