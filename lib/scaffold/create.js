...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'berycoincore-lib': 'https://github.com/berycoin-project/berycoincore-lib.git#master',
    'berycoincore-node': 'https://github.com/berycoin-project/berycoincore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/berycoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the berycoincore-node directory and configuration
